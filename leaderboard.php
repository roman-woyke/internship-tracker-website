...
$actStmt = $pdo->prepare("
    ...
    FROM applications WHERE user_id = ? AND DATE(created_at) >= ?
    GROUP BY DATE(created_at)
");
$actStmt->execute([$userId, $monday]);
$actByDay = [];
foreach ($actStmt->fetchAll(PDO::FETCH_ASSOC) as $r) { $actByDay[$r["day"]] = (int) $r["cnt"]; }
...
<head>
    ...
    <title>intern.track — Leaderboard</title>
